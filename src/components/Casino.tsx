...
export default function Casino() {
  ...
  const сыгратьВСлоты = () => {
    ...
    // ВСЕ СИМВОЛЫ С ВЕРОЯТНОСТЯМИ
    const getSymbol = () => {
      const rand = Math.random() * 100
      if (rand < 2) return '7️⃣'      // 2% - семёрка (x36)
      if (rand < 6) return 'BAR'      // 4% - BAR (x16)
      if (rand < 10) return '🍋'      // 4% - лимон (x16)
      if (rand < 14) return '🍒'      // 4% - вишня (x2)
      if (rand < 24) return '⭐'      // 10% - звезда (шлак)
      if (rand < 34) return '🔔'      // 10% - колокол (шлак)
      if (rand < 44) return '💎'      // 10% - бриллиант (шлак)
      if (rand < 54) return '🎰'      // 10% - казино (шлак)
      if (rand < 64) return '🍉'      // 10% - арбуз (шлак)
      return '🍒'                      // остальное - вишня
    }
    ...
    if (reel1 === '7️⃣' && reel2 === '7️⃣' && reel3 === '7️⃣') {
      ...
      // ШЛАК - разные символы или пустые комбинации
      выигрыш = 0
      сообщение += `😞 Вы проиграли $${ставка}!`
    }
